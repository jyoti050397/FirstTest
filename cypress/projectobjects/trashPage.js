export default class TrashPage {
    ...
    //  viewprofile:()=>cy.findAllByText('View Profile'),
     ...
     threedots:()=>cy.get('.cu-trash__item-actions-icon > .ng-star-inserted'),
    ...
    clickTrashButton() {
      ...
      urlAssertion() {
        cy.url().should('include', '/9002137880/settings/trash', { timeout: 10000, retryInterval: 500 });
      }
      Searchbar() {
    ...
    delteby() {
      ...
      threedots() {
        this.elements.threedots().click({ force: true});
      }
      Restore() {
